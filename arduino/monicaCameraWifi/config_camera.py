...
class MicroROS_Robot():
    ...
    # Configure the ESP WiFi information, enter the ESP WiFi signal name and password. Restart takes effect.
    def set_esp_config(self, ssid, passwd):
        """
        配置WiFi信息, 输入WiFi信号名称和密码。重启生效
        输入参数示例: ssid="ESP_WIFI", passwd="ESP_PASSWD"
        """
        esp_ssid_bytes = bytes(str(ssid), "utf-8")
        for i in range(len(ssid)):
            ORDER["ESP_SSID"].append(esp_ssid_bytes[i])
        self.__send("ESP_SSID", len=len(ssid))
        esp_passwd_bytes = bytes(str(passwd), "utf-8")
        for i in range(len(passwd)):
            ORDER["ESP_PASSWD"].append(esp_passwd_bytes[i])
        self.__send("ESP_PASSWD", len=len(passwd))
    ...
    def read_esp_ssid(self):
        '''
        读取底板连接的WiFi信号名称
        '''
        self.__request(ORDER["ESP_SSID"][0])
        time.sleep(self.__read_delay)
        str_data = None
        if self.__unpack():
            str_data = self.__rx_DATA.decode('utf-8')
    ...
    def read_esp_passwd(self):
        '''
        读取底板连接的WiFi密码
        '''
        self.__request(ORDER["ESP_PASSWD"][0])
        time.sleep(self.__read_delay)
        str_data = None
        if self.__unpack():
            str_data = self.__rx_DATA.decode('utf-8')
    ...
    def print_all_firmware_parm(self):
        ...
        esp_ssid = self.read_esp_ssid()
        print("esp_ssid:", esp_ssid)
        esp_passwd = self.read_esp_passwd()
        print("esp_passwd:", esp_passwd)
    ...
    def print_esp_init_message(self):
        line = self.__ser.read_all()
        print(line)
...
if __name__ == '__main__':
    ...
    robot.set_esp_config("ESP_WIFI", "esp123")
    #robot.set_udp_config([192, 168, 2, 249], 8090)
    ...
    print("Wait to reboot camera and get Camera Info Log. Please wait 3~5seconds.")

    time.sleep(.1)
    robot.reboot_reset()
    print("remain 3 seconds")
    time.sleep(1)
    print("remain 2 seconds")
    time.sleep(1)
    print("remain 1 seconds")
    time.sleep(1)
    robot.print_esp_init_message()
